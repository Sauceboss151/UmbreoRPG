...
import net.mcreator.umbreorpg.procedures.SkillCommandProcedureProcedure;
...
public class SkillChangeCommandCommand {
	@SubscribeEvent
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("skill")
				...
				.then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(arguments -> {
					...
					SkillCommandProcedureProcedure.execute(world, entity, cmdparams);
					return 0;
				})).executes(arguments -> {
					ServerLevel world = arguments.getSource().getLevel();
					double x = arguments.getSource().getPosition().x();
