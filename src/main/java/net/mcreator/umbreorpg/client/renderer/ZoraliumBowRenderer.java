...
public class ZoraliumBowRenderer extends EntityRenderer<ZoraliumBowEntity> {
	...
	public ZoraliumBowRenderer(EntityRendererProvider.Context context) {
		super(context);
		model = new Modelumbreorpg_arrow(context.bakeLayer(Modelumbreorpg_arrow.LAYER_LOCATION));
	}

	@Override
	public void render(ZoraliumBowEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn,
			int packedLightIn) {
	...
	@Override
	public ResourceLocation getTextureLocation(ZoraliumBowEntity entity) {
		return texture;
	}
}
