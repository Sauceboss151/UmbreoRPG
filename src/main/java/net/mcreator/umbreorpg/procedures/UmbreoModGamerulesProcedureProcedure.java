		...
		if (!(new Object() {
				...
				if (!world.isClientSide()) {
					...
					if (_mcserv != null)
						_mcserv.getPlayerList().broadcastMessage(
								new TextComponent("Reduced Debug Info will always be set TRUE with this Mod Installed"), ChatType.SYSTEM,
								Util.NIL_UUID);
				}
			}
			if (world.getLevelData().getGameRules().getBoolean(GameRules.RULE_DO_TRADER_SPAWNING) == true) {
				...
				if (!world.isClientSide()) {
					...
					if (_mcserv != null)
						_mcserv.getPlayerList().broadcastMessage(
								new TextComponent("Do Trader Spawning will always be set FALSE with this Mod Installed"), ChatType.SYSTEM,
								Util.NIL_UUID);
				}
			}
			if (world.getLevelData().getGameRules().getBoolean(GameRules.RULE_DO_PATROL_SPAWNING) == true) {
				...
				if (!world.isClientSide()) {
					...
					if (_mcserv != null)
						_mcserv.getPlayerList().broadcastMessage(
								new TextComponent("Do Patrol Spawning will always be set FALSE with this Mod Installed"), ChatType.SYSTEM,
								Util.NIL_UUID);
				}
			}
			if (world.getLevelData().getGameRules().getBoolean(GameRules.RULE_LIMITED_CRAFTING) == false) {
				...
				if (!world.isClientSide()) {
					...
					if (_mcserv != null)
						_mcserv.getPlayerList().broadcastMessage(
								new TextComponent("Do Limited Crafting will always be set TRUE with this Mod Installed"), ChatType.SYSTEM,
								Util.NIL_UUID);
				}
			}
		} else {
