...
import net.mcreator.umbreorpg.network.WorldStateKeybindMessage;
...
public class UmbreoRpgModKeyMappings {
	...
	public static final KeyMapping WORLD_STATE_KEYBIND = new KeyMapping("key.umbreo_rpg.world_state_keybind", GLFW.GLFW_KEY_M, "key.categories.misc");

	@SubscribeEvent
	public static void registerKeyBindings(FMLClientSetupEvent event) {
		ClientRegistry.registerKeyBinding(OPEN_SKILL_GUI);
		ClientRegistry.registerKeyBinding(WORLD_STATE_KEYBIND);
	}
	...
	public static class KeyEventListener {
		...
		public static void onKeyInput(InputEvent.KeyInputEvent event) {
			if (Minecraft.getInstance().screen == null) {
				...
				if (event.getKey() == WORLD_STATE_KEYBIND.getKey().getValue()) {
					if (event.getAction() == GLFW.GLFW_PRESS) {
						UmbreoRpgMod.PACKET_HANDLER.sendToServer(new WorldStateKeybindMessage(0, 0));
						WorldStateKeybindMessage.pressAction(Minecraft.getInstance().player, 0, 0);
					}
				}
			}
		}
